...
app.get('/user/tweets/feed/', authentication, async (request, response) => {
  ...
  INNER JOIN tweet ON user.user_id = tweet.user_id WHERE user.user_id IN ($(followingPeopleIds)) ORDER BY date_time DESC LIMIT 4;`
...
app.get('/user/following/', authentication, async (request, response) => {
  ...
  const getFollowingUsersQuery = `SELECT name FROM follower INNER JOIN user ON 
...
app.get('/user/followers/', authentication, async (request, response) => {
  ...
  user ON user.user_id = follower.follower_user_id WHERE following_user_id = "${userId}"; `
